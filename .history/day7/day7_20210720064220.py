...
def get_number_of_individual_bags(all_bags, wanted_bag, total_number):
    if all_bags[wanted_bag].values() is None:
        return 0
    else:
        for key, value in all_bags[wanted_bag].items():
            subtotal_number = int(value)
        print(subtotal_number)
...
for key, value in all_bags.items():
    if key == 'shiny gold':
        ...
        total_number += get_number_of_individual_bags(all_bags, 'shiny gold', total_number)
        ...
        for inside_bag, number in value.items():
            ...
            total_number += get_number_of_individual_bags(all_bags, wanted_bag, total_number)
